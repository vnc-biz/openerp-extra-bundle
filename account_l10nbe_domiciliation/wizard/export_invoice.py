...
<separator string="Export invoice to DOM80 format" colspan="4" />
   ...
   <field name="msg1"  colspan="4" />
   <field name="msg2" colspan="4" />
      </form>"""
...
fields = {
    ...
    'collection_date' : {
        ...
        'msg1' : {
        'string':'Message to the payer, part 1.', 
        'type':'char', 
        'size' : 15
    },
        'msg2' : {
        'string':'Message to the payer, part 2.', 
        'type':'char', 
        'size' : 15
    },
    }
...
def _create_file(self, cr, uid, data, context):
    ...
    code = obj_cmpny.partner_id.bank_ids and obj_cmpny.partner_id.bank_ids[0].institution_code
    if not code:
        return {'note':'Please provide Institution Code number for the Sender.', 'invoice_file': False, 'state':'failed' }
    
    v1['institution_code'] = code # 12-14
    v1['app_code']= '02' # 15-16
    v1['ref_file']= data['form']['ref_file'] or '' # 17-26
    
    id_sender = obj_cmpny.id_sender
    if not id_sender:
        return {'note':'Please Provide VAT number for the Sender.', 'invoice_file': False, 'state':'failed' }
    v1['id_sender']= id_sender # 27-37

    id_creditor = obj_cmpny.id_creditor
    if not id_creditor:
        return {'note':'Please Provide VAT number for the creditor.', 'invoice_file': False, 'state':'failed' }
    v1['id_creditor']= id_creditor
    
    #Taken bank account num of main company
    partner_bank = obj_cmpny.partner_id.bank_ids and obj_cmpny.partner_id.bank_ids[0].acc_number
    
    if partner_bank:
        v1['acc_num_creditor'] = partner_bank
    else:
        return {'note':'Please Provide Bank number for the creditor.', 'invoice_file': False, 'state':'failed' }
    ...
    cr.execute('select id from account_invoice where domiciled=True and domiciled_send_date is null')
    inv_ids = map(lambda x:x[0], cr.fetchall())
        
    for inv in invoice_obj.browse(cr, uid, inv_ids):
        seq=seq+1
        v2['identification'] = '1'
        v2['serial_num'] = str(seq).rjust(4, '0')
        v2['direct_debit_num'] =  (inv.partner_bank and  inv.partner_bank.acc_number) \
        		or  (inv.partner_id.bank_ids and inv.partner_id.bank_ids[0].acc_number)
        direct_debit_num_tot += int(v2['direct_debit_num'])
        v2['type_code'] = '0' 
        v2['amount_collection'] = (('%.2f' % inv.amount_total).replace('.', '')).rjust(12, '0')
        v2['creditor'] = inv.partner_id.name#.ljust(26 , ' ')
        v2['msg_payer_1'] = data['form']['msg1'] or ''
        v2['msg_payer_2'] = data['form']['msg2'] or ''
    ...
    #Trailer Record Start
    v3['identification'] = '9'
    v3['tot_collection'] = str(seq).rjust(4, '0')
    ...
    v3['tot_reversal_inst'] = ''.rjust(4, '0')
    v3['tot_amount_reversal_inst']  = ''.rjust(12, '0')
    v3['tot_reversal_direct_debit_num']  = ''.rjust(15, '0')
    ...
    return {'note':log(), 'invoice_file': base64.encodestring(dom_data), 'state':'succeeded'}
