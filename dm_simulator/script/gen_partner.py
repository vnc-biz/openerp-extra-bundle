...
for i in range(1, int(no_partner)+1):
    p_id= server.execute(db, uid, pwd, 'res.partner', 'create', 
                         {'name': str('P%%0%sd' % padding % i)})
    a_id= server.execute(db, uid, pwd, 'res.partner.address', 'create',
                          {'name': str('Address of P%%0%sd' % padding % i), 
                           'partner_id': p_id})
#    print p_id, a_id
