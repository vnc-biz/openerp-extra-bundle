        ...
        for picking in self.browse(cr, uid, ids, context=context):
            ...
            if not partner:
                raise osv.except_osv(_('Error, no partner !'),
                    _('Please put a partner on the picking list if you want to'\
                      ' generate invoice.'))
            ...
            address = address_obj.browse(cr, uid, address_contact_id,
                                                                context=context)
            ...
            if group and partner.id in invoices_group:
                ...
                invoice_vals = {
                    ...
                    'comment': (comment and (invoice.comment \
                            and invoice.comment + "\n" + comment or comment)) \
                            or (invoice.comment and invoice.comment or ''),
                    'date_invoice':context.get('date_inv', False),
                    'user_id':uid
                }
                invoice_obj.write(cr, uid, [invoice_id], invoice_vals,
                                                            context=context)
            else:
                invoice_vals = {
                    'name': picking.name,
            ...
            for move_line in picking.move_lines:
                ...
                account_analytic_id = self._get_account_analytic_invoice(cr,
                                                        uid, picking, move_line)
                ...
                account_obj = self.pool.get('account.fiscal.position')
                account_id = account_obj.map_account(cr, uid,
                ...
                invoice_line_id = invoice_line_obj.create(cr, uid, {
                    ...
                    'quantity': move_line.product_uos_qty \
                                                    or move_line.product_qty,
                    'invoice_line_tax_id': [(6, 0, tax_ids)],
                    'account_analytic_id': account_analytic_id,
                    'note': move_line.product_id.description_sale,
                    'price_unit_picking': price_unit_picking,
                    'quantity_picking': quantity_picking,
                }, context=context)
