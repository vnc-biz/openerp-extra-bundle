            ...
            rml=""" <document filename="test.pdf">
                    <template pageSize="(1120.5,767.8)" title="Test" author="Martin Simon" allowSplitting="20" >
                        <pageTemplate id="first">
                          ...
                          <!--pageGraphics>
                <letterBoxes x="12.58cm" y="6.80cm"
                 lineWidth="0.0002mm"
                 boxWidth="0.6cm" boxHeight="0.5cm"
                 boxFillColor="black" boxStrokeColor="black"
                 fontName="Helvetica" fontSize="10"
                 spaceBetweenBox="1.0mm" removeSpace="0"
                 count="8" align ="right">[[ str(get_value(o.amount_total)) ]]</letterBoxes></pageGraphics-->
                    </pageTemplate>
  </template>
  <stylesheet>
  ...
  <story>"""
            for page in survey.page_ids:
                ...
                for que in page.question_ids:
                    ...
                    <blockTable colWidths="1000.0" style="Table5">
                              <tr>
                                <td><para style="question">Que: """+ que.question + """</para></td>
                              </tr>
                             </blockTable>
                             <para style="P2"><font></font></para>"""
                    if que.type in ['multiple_choice_multiple_ans','multiple_choice_only_one_ans']:
                            ...
                            for div in range(0,len(divide)):
                               if divide[div]!='':
                                   if que.type == 'multiple_choice_multiple_ans':
                                           ...
                                           rml+="""<td><illustration>
                                                          <rect x="0.1cm" y="-0.4cm" width="0.5 cm" height="0.5cm"
                                                                fill="no" stroke="yes"/></illustration></td>
                                                   <td><para style="answer">""" + divide[div] + """</para></td>"""

                                   else:
                                       rml+="""<td><illustration>
                                                     <circle x="0.3cm" y="-0.18cm" radius="0.23 cm" fill="no"
                                                          stroke="yes"/></illustration></td>
                                               <td><para style="answer">""" + divide[div] + """</para></td>"""
                               else:
                                   rml+="""<td></td>
                                           <td></td>"""
                        ...
                        for mat_col in matrix_ans:
                            rml+="""<td><para style="response">""" + mat_col + """</para></td>"""
                        rml+="""</tr>"""
                        for ans in que.answer_choice_ids:
                            rml+= """<tr>"""
                            ...
                            for mat_col in range(1,len(matrix_ans)):
                                rml+="""<td><illustration>
                                             <rect x="0.25cm" y="-0.5cm" width="0.8 cm" height="0.5cm"
                                                    fill="no" stroke="yes" round="0.1cm"/>
                                                                </illustration></td>"""
                            rml+= """</tr>"""
                        rml+="""</blockTable>"""
                    else:
                        ...
                        <blockTable colWidths=" """ + colWidths + """ " style="Table1">
                        <tr>
                        <td><illustration>
                                     <rect x="0.2cm" y="0.3cm" width="15.0 cm" height="0.6cm"
                                            fill="no" stroke="yes"/>
                                                        </illustration></td>
                        </tr>
                        </blockTable>
